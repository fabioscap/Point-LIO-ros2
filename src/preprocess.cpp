...
void Preprocess::process_cut_frame_pcl2(
  ...
  if (lidar_type == VELO16) {
    ...
    for (int i = 0; i < plsize; i++) {
      ...
      added_pt.intensity = pl_orig.points[i].intensity;
      added_pt.curvature =
          (pl_orig.points[i].timestamp - msg_time) * 1000;  // s to ms
  ...
  // ms
  double last_frame_end_time = msg_time * 1000;
  uint valid_num = 0;
  uint cut_num = 0;
  ...
  for (uint i = 1; i < valid_pcl_size; i++) {
    valid_num++;
    pl_surf[i].curvature += msg_time * 1000 - last_frame_end_time;
...
void Preprocess::oust64_handler(
  ...
  double time_stamp = stamp_to_sec(msg->header.stamp);
  (void)time_stamp;
  // cout << "===================================" << endl;
  ...
  for (size_t i = 0; i < pl_orig.points.size(); i++) {
    ...
    if (range < (blind * blind) || range > det_range * det_range ||
...
void Preprocess::velodyne_handler(
  ...
  if (pl_orig.points[plsize - 1].time > 0) {
    ...
    // * 57.29578; double yaw_end  = yaw_first; int layer_first =
    // pl_orig.points[0].ring; for (uint i = plsize - 1; i > 0; i--)
    // {
  ...
  for (int i = 0; i < plsize; i++) {
    ...
    if (i % point_filter_num != 0 || std::isnan(added_pt.x) ||
    ...
    if (!given_offset_time) {
      ...
      // cout << "check layer:" << unit_size << ";" << i << ";" << layer <<
      // endl;
      int layer = 0;
      double yaw_angle = atan2(added_pt.y, added_pt.x) * 57.2957;
...
bool Preprocess::small_plane(const PointCloudXYZI& pl, vector<orgtype>& types,
                             uint i_cur, uint& i_nex,
                             Eigen::Vector3d& curr_direct) {
  (void)pl;
  (void)types;
