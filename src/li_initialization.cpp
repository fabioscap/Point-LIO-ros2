...
// Helper function to get seconds from ROS2 timestamp
inline double stamp_to_sec(const builtin_interfaces::msg::Time& stamp)
{
    return static_cast<double>(stamp.sec) + static_cast<double>(stamp.nanosec) * 1e-9;
}

// Helper function to create ROS2 timestamp from seconds
inline builtin_interfaces::msg::Time sec_to_stamp(double seconds)
{
...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    ...
    double msg_time = stamp_to_sec(msg->header.stamp);
    ...
    if (con_frame)
    {
        if (frame_ct == 0)
        {
            time_con = last_timestamp_lidar; //msg_time;
        }
        if (frame_ct < 10)
        {
...
void imu_cbk(const sensor_msgs::msg::Imu::SharedPtr msg_in)
{
    ...
    double original_time = stamp_to_sec(msg->header.stamp);
    double adjusted_time = original_time - timediff_imu_wrt_lidar - time_lag_IMU_wtr_lidar;
    msg->header.stamp = sec_to_stamp(adjusted_time);
    ...
    {
        RCLCPP_ERROR(g_node->get_logger(), "imu loop back, clear deque");
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    double imu_next_time = stamp_to_sec(imu_next.header.stamp);
    ...
    if (!lose_lid && !imu_pushed)
    {
        ...
        {
            double imu_time = stamp_to_sec(imu_deque.front()->header.stamp);
            ...
            while (imu_time < lidar_end_time)
            {
                ...
                imu_time = stamp_to_sec(imu_deque.front()->header.stamp); // can be changed
                imu_next = *(imu_deque.front());
            }
        }
        imu_pushed = true;
